...
class FANSegmentator(object):
    ...
    def __init__ (self, resolution, face_type_str, load_weights=True, weights_file_root=None, training=False):
        ...
        if training:
            ...
            loss = K.mean(K.binary_crossentropy(real_t,out_t) )

            out_t_diff1 = out_t[:, 1:, :, :] - out_t[:, :-1, :, :]
            out_t_diff2 = out_t[:, :, 1:, :] - out_t[:, :, :-1, :]

            total_var_loss = K.mean( 0.01*K.abs(out_t_diff1), axis=[1, 2, 3] ) + K.mean( 0.01*K.abs(out_t_diff2), axis=[1, 2, 3] )

            opt = Adam(lr=0.001, beta_1=0.5, beta_2=0.999, tf_cpu_mode=2)
